...
export default function Student() {
  ...
  return (
    ...
    <h2 class="text-2xl font-bold mb-4 text-gray-700">Student Details</h2>
      ...
      <table class="min-w-full bg-white shadow-md rounded-lg overflow-hidden">
        ...
        <tbody class="text-gray-700">
            ...
            <tr key={s.sid} class="border-b hover:bg-gray-100">
                ...
                <td class={`py-3 px-6 ${s.age>22 ? 'bg-green-500' : 'bg-red-500'}`}>{s.age}</td>
                <td class="py-3 px-6">{FormatDate(new Date(s.dob))}</td>
                <th class="py-3 px-6 text-left flex">
                 <FaEdit className='hover:text-red-500' onClick={()=>navigate('/admin/student/editStudent',{state:s})}/>
                <FaRegTrashAlt className='hover:text-red-500 ml-4' onClick={()=>handleDelete(s.sid)}/></th>
            </tr>
            ))}
